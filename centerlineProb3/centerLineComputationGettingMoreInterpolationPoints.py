...
pointsWithIds_sph = []
with open('particles_initSPH.csv', 'rb') as csvfile:
...
with open('particles_endSPH.csv', 'rb') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
	rowNumber = 0
	for row in spamreader:
		if(rowNumber > 1):
...
#		xSecondPlot.append(listOfDeformedPoints[listOfFoundIdsInOrderSecondPlot[i]][1][0])
	...
	plt.plot(xSecondPlot, deformedPointsZHighResolution, 'o')

def addingSubPlotFEM(listOfPointsWithIds, listOfDeformedPoints):
	listOfFoundIdsInOrder = []
	listOfFoundIdsInOrderSecondPlot = []
	deformedPointsZHighResolution = []
	deformedPointsYHighResolution = []
	...
	numOfPoints = 30
	for i in range(resolutionOfSecondPlot):		
		closestpoints_id_sph = findIdWithMinDistance(listOfPointsSecondPlot[i], listOfPointsWithIds, numOfPoints)
		listOfFoundIdsInOrderSecondPlot.append(closestpoints_id_sph)
...
#		print listOfDeformedPoints[listOfFoundIdsInOrderSecondPlot[i]], listOfDeformedPoints[listOfFoundIdsInOrderSecondPlot[i]][1][2]
		print "\n"
		sumOfDists = 0
		powerOfInverseDistances = 1
		closestIdList = []
		distancesList = []
		zOfClosestIdList = []
		zOfClosestIdUndeformedList = []
		yOfClosestIdList = []
		xOfClosestIdList = []
		xOfClosestIdUndeformedList = []
		for interPoint in range(numOfPoints):
			closestIdList.append(listOfFoundIdsInOrderSecondPlot[idsIndex][interPoint][2])
			dist = pow(1. / listOfFoundIdsInOrderSecondPlot[idsIndex][interPoint][0], powerOfInverseDistances)
			distancesList.append(dist)
			sumOfDists = sumOfDists + dist;
		for interPoint in range(numOfPoints):
			distancesList[interPoint] = distancesList[interPoint]/sumOfDists
			zOfClosestIdList.append(0)
			yOfClosestIdList.append(0)
			zOfClosestIdUndeformedList.append(0)
			xOfClosestIdList.append(0)
			xOfClosestIdUndeformedList.append(0)
		for i in range(len(listOfPointsWithIds)):
#			print listOfPointsWithIds[i][0], listOfFoundIdsInOrderSecondPlot[idsIndex]
			sumOfZs = 0
			sumOfZUndeformed = 0
			...
			for interPoint in range(numOfPoints):
				if(listOfPointsWithIds[i][0] == closestIdList[interPoint]):
					zOfClosestIdList[interPoint] = (listOfDeformedPoints[i][1][2])
					yOfClosestIdList[interPoint] = (listOfDeformedPoints[i][1][1]) 
					zOfClosestIdUndeformedList[interPoint] = (listOfPointsWithIds[i][1][2])
					xOfClosestIdList[interPoint] = (listOfDeformedPoints[i][1][0])					
					xOfClosestIdUndeformedList[interPoint] = (listOfPointsWithIds[i][1][0])
		for interPoint in range(numOfPoints):
			sumOfZs = sumOfZs + zOfClosestIdList[interPoint] * distancesList[interPoint]
			sumOfYs = sumOfYs + yOfClosestIdList[interPoint] * distancesList[interPoint]
			sumOfXs = sumOfXs + xOfClosestIdList[interPoint] * distancesList[interPoint]
			sumOfXUndeformed = sumOfXUndeformed + xOfClosestIdUndeformedList[interPoint] * distancesList[interPoint]
			sumOfZUndeformed = sumOfZUndeformed + zOfClosestIdUndeformedList[interPoint] * distancesList[interPoint]
		
		deformedPointsZHighResolution.append(sumOfZs)
		deformedPointsYHighResolution.append(sumOfYs)
		undeformedPointsZHighResolution.append(sumOfZUndeformed)
		xSecondPlot.append(sumOfXs)
		xSecondPlotUndeformed.append(sumOfXUndeformed)
...
#	l2, = plt.plot(listOfPointsX, listOfPointsZ, 'gray')
#	plt.plot(listOfPointsX, listOfPointsZ, 'o')

#	plt.show()
#	l3 = plt.plot(xSecondPlot, deformedPointsZHighResolution, '--y^', ms = 10, linewidth = 3)
	l4, = plt.plot(xSecondPlot, deformedPointsZHighResolution, '--y^', ms = 10, linewidth = 3)
#	plt.plot(xSecondPlot, deformedPointsYHighResolution, '', ms = 10)
	return l4
#	plt.plot(xSecondPlotUndeformed, undeformedPointsZHighResolution, 'blue')
#	plt.plot(xSecondPlotUndeformed, undeformedPointsZHighResolution, 'o')
...
def addingSubPlotSPH(listOfPointsWithIds, listOfDeformedPoints):
	...
	numOfPoints = 30
	for i in range(resolutionOfSecondPlot):		
		closestpoints_id_FEM = findIdWithMinDistance(listOfPointsSecondPlot[i], listOfPointsWithIds, numOfPoints)
		listOfFoundIdsInOrderSecondPlot.append(closestpoints_id_FEM)
...
#	l2, = plt.plot([-8,0],[0,0], color='gray', linestyle='-', linewidth = 3)
#	l3 = plt.plot(xSecondPlot, deformedPointsZHighResolution, '-bo', ms = 10, linewidth = 3)

#	print xSecondPlotUndeformed
#	print listOfPointsX
	l5, = plt.plot(xSecondPlot, deformedPointsZHighResolution, '-bo', ms = 10, linewidth = 3)
...
def addingSubPlotsOfDeformation(undeformedFileNameFolder):
	...
	l1, = plt.plot(xSecondPlot, deformedPointsZHighResolution,'b', linewidth = 3)
	plt.plot(xSecondPlot, deformedPointsZHighResolution, 'bo', ms = 10)
...
#		print deformedPoint[0], deformedPoint[2]
	...
	x = [1, 2, 3, 4, 5, 6, 7, 8, 9]
	labelsPoints = ["p1","p2","p3","p4","p5","p6","p7","p8","p9"]
	plt.plot(x, listOfStrainsInX, 'bo', ms = 10)
...
def addingSubPlotFEMStrain1(undeformedFileNameFolder):
	undeformed = "/undeformed.vtk"
	undeformedFileName = undeformedFileNameFolder + undeformed
	deformed = "/deformed.vtk"
	deformedFileName = undeformedFileNameFolder + deformed
...
plot_lines = []					
for root, directories, filenames in os.walk('/home/eric/scriptsToHelpSPHCreateGeometryInteraceETC/centerline/meshesWithMoreDegreesOfFreedomProb3'):
...
#			l1 = addingSubPlotsOfDeformation(undeformedFileNameFolder)
			l3 = addingSubPlotFEM(pointsWithIds_FEM, deformedPointsWithIds_FEM)
			l2, l1 = addingSubPlotSPH(pointsWithIds_sph, deformedPointsWithIds_sph)

			plot_lines.append([l3, l2, l1])
...
for root, directories, filenames in os.walk('/home/eric/scriptsToHelpSPHCreateGeometryInteraceETC/centerline/meshesWithMoreDegreesOfFreedomProb3'):
	for filename in filenames:
		if(filename == 'undeformed.vtk'):
			undeformedFileNameFolder = os.path.join(root)
#			print undeformedFileNameFolder
			#listOfStrainsFEM = addingSubPlotFEMStrain(undeformedFileNameFolder)
...
#				 plt.plot(x, listOfStrainsFEM[graphicToPlot],'y--', linewidth = 3)
#				plt.xticks(x, labelsPoints, rotation='vertical')
#				plt.margins(0.2)
#				plt.subplots_adjust(bottom=0.15)
#
#				l2, = plt.plot(x, listOfStrainsSPH[graphicToPlot], '-bo', ms = 10, linewidth = 3)
#				l2, = plt.plot(x, listOfStrainsSPH[graphicToPlot],'b', linewidth = 3)
#				plt.xticks(x, labelsPoints, rotation='vertical')
#				plt.margins(0.2)
#				plt.subplots_adjust(bottom=0.15)
#
...
#
#				if(graphicToPlot < 4 or graphicToPlot == 7):
