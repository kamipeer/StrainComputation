...
def transfer_info_points2cells_threshold(mesh, arrayname_points, arrayname_cells, threshold):
    # transfer the information of an array from point to cell using the threshold to separate
    # threshold = 1.5 in the case of scar (porque esa scar ya no es binaria despues del registro de la malla)
    ...
    #transfer the information about the array from points to cells
    #create scar_cell_array (if the sum of scar is > 1.5 then it's marked as scar)
    for j in range(ncells):
    # get the 3 point ids
        ptids = mesh.GetCell(j).GetPointIds()
        if (ptids.GetNumberOfIds() != 3):
            print "Non triangular cell"
        val0 = mesh.GetPointData().GetArray(arrayname_points).GetTuple1(ptids.GetId(0)) # read scar info in point with id0
        val1 = mesh.GetPointData().GetArray(arrayname_points).GetTuple1(ptids.GetId(1)) # esto no son valores enteros! interpolacion por el registro
        val2 = mesh.GetPointData().GetArray(arrayname_points).GetTuple1(ptids.GetId(2))
        suma = (val0 + val1 + val2)
        #print suma (min=0, max=3)
        if (suma > threshold):
...
def resample_ray(polydata, m):  # take a ray and taking first and last point interpolates m values in the middle (returns new polydata)
...
def scar_segmentation_disk(polydata, inarrayname, outarrayname, th): # lee array inarrayname ('scalars'), aplica el threshold que sea y guarda resultado en outarrayname, binario (0-> healthy,  1-> scar) 
...
def convert_to_poly(unstructured_grid):
    ...
    geometryFilter.SetInput(unstructured_grid)
...
def centroidofcentroidsrefdist(edges,refpoint,refdist=1000):
    ...
    for r in range(rn):
        ...
        if dist < refdist:
            ...
            rncnt = rncnt - 1
            print "skiping this edge",dist,'total edges',rncnt
        # print acumvector
...
def computeclippointnormal(clenabled,clids,factor):
    ...
    for j in clids:
        ...
        print "startsphere",sphere_array.GetValue(startid),"averageshpere",averageshpere
        ...
        print "target",targetsphere,"averageshpere",averageshpere
        ...
        while (currentabscissa <= targetsphere):
            ...
            print 'start',startabscissa,'current',currentabscissa,'target',targetsphere
...
def clip_disk_with_circle(diskpolydata,normal,center,rad,scale,inside=0):
    ...
    print rotationaxis,rotationangle
    transform = vtk.vtkTransform()
...
def extractnonregion(polydata,ref,refdist):
    ...
    for r in range(numberofpossibleregions):
        ...
        print "for region", r , "dist", dist
        if dist < mindist:
            mindist = dist
            selectedr = r
            print "selected region", selectedr,"distance",mindist
    ...
    for r in range(numberofpossibleregions):
        if r != selectedr:
            ...
            for i in range(possibleregion.GetNumberOfPoints()):
                ...
                acumdist = acumdist + euclideandistance(point0,point1)
            dist = acumdist /possibleregion.GetNumberOfPoints()
            print "for region", r, "dist", dist, refdist
            if dist > refdist:
                print "keeping region", r
                appender.AddInput(possibleregion)
                appender.Update()
...
def findadjoiningregionid(reference, target):
    ...
    if nregions > 0:
        for r in range(int(nregions[1])+1):
            ...
            print "currentdist",currentdist,"smallestdist",smallestdist
            if currentdist < smallestdist:
                smallestdist = currentdist
                smallestdistr = r
            print "smallest region",smallestdistr
...
def getconnectedvertices(mesh,pointid):
    ...
    print "End points are " , pointIdList.GetId(0) , " and " ,pointIdList.GetId(1)

    if pointIdList.GetId(0) != pointid:
      print "Connected to " , pointIdList.GetId(0)
      connectedVertices.InsertNextId(pointIdList.GetId(0))
    else:
      print "Connected to " , pointIdList.GetId(1)
...
def sphereclip(polydata, center, radius):
...
def visualise_slice_plus_mesh(imagefile,surface,point,size,position,focalpoint,
    ...
    print "cutting at",pointclip, normal
...
def visualise(surface,ref,case,arrayname,mini,maxi,viewup,position,focalpoint,clipping,scale,size,interact=1,filename='out.png'):
    ...
    if interact ==1:
        ...
        print "position",outcam.GetPosition()
        print "focalpoint",outcam.GetFocalPoint()
        print "parallel scale",outcam.GetParallelScale()
        print "viewup",outcam.GetViewUp()
        print "clip",outcam.GetClippingRange()
...
def visualise_default_continuous(surface,overlay,case,arrayname,edges=0,flip=0,colormap='BlYl',
    ...
    print "From",mini,"to",maxi, "colormap",colormap
    ...
    if flip ==1:
        print "flip to foot to head position"
